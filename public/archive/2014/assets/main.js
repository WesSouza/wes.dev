...
var Wesley = function() {
  ...
  self.init = function() {
    ...
    if ($('#unwex').length) self.unwex = new Wesley_UnWex(self);
  };
...
var Wesley_UnWex = function(parent) {
  ...
  self.enable = function(e) {
    ...
    if (!e || e.type != 'focus') $capture.focus();
  };
  ...
  self.eventKeyDown = function(e) {
    if (!enabled || self.eventReject(e)) return;
    ...
    if (self.scanfing) {
      ...
      self.scrollToBottom();
      if (
        (modifier == 0 && [9, 38, 40].indexOf(key) != -1) ||
  ...
  self.eventKeyUp = function(e) {
    ...
    if (self.scanfing) {
      self.updateCommand();
      if (modifier == 0 && key == 13)
        // return
        self.scanfReturn();
      if (modifier == 2 && key == 76)
        // Ctrl+L
        self.clear();
      if (modifier == 2 && key == 67)
        // Ctrl+C
        self.sigkill();
      if (modifier == 0 && key == 38)
        // Up
        self.scanfHistoryNavigate(-1);
      if (modifier == 0 && key == 40)
        // Down
        self.scanfHistoryNavigate(1);
      if (modifier == 0 && key == 37)
        // Left
        self.scanfMoveCursor();
      if (modifier == 0 && key == 39)
        // Right
        self.scanfMoveCursor();
      if (modifier == 2 && key == 65)
        // Ctrl+A
        self.scanfMoveCursor(0);
      if (modifier == 2 && key == 69)
        // Ctrl+E
        self.scanfMoveCursor('end');
      if (modifier == 2 && key == 85)
        // Ctrl+U
        self.scanfEmpty();

      if (modifier == 0 && key == bsodTrigger[bsodCurrent + 1]) bsodCurrent++;
      else bsodCurrent = -1;

      if (bsodCurrent >= bsodTrigger.length - 1) self.bsod(bsodText);
    } else $capture.val('');
  };
  ...
  self.scanfHistoryNavigate = function(direction) {
    if (
      !scanfHistoryEnabled ||
      (direction == -1 && scanfHistoryIndex == 0) ||
      (direction == 1 && scanfHistoryIndex == scanfHistory.length - 1)
    )
      return;
  ...
  self.scanfMoveCursor = function(position) {
    if ($capture[0].selectionStart === undefined) return;
    if (position == 'end') position = $capture.val().length;
    if (position !== undefined)
      $capture[0].selectionStart = $capture[0].selectionEnd = position;
    debug(
      'scanf move cursor',
      $capture.val().length,
      $capture[0].selectionStart,
    );
    $cursor.css(
      'left',
      '-' +
        ($capture.val().length - $capture[0].selectionStart) *
          $cursor.outerWidth() +
        'px',
    );
  };
  ...
  self.scanfReturn = function() {
    ...
    })(self.scanfCallback, $command.text());

    self.scanfing = false;
    self.scanfCallback = null;
    self.writeln(
      $prompt.text() + ($command.is(':visible') ? $command.text() : ''),
    );
    $prompt.text('');
    $command.text('').show();
    $capture.val('');
  ...
  self.writeAnimated = function(text, callback) {
    ...
    timer = setInterval(function() {
      ...
      else if (nextChar == '\r') pause = pauseForPause;
      else if (nextChar == ' ') pause = pauseForSpace;

      self.write(nextChar);

      if (text.length == 1) pause = pauseForEOL;
  ...
  function modifierKeys(event) {
    var result = 0;
    if (event.shiftKey) result += 1;
    if (event.ctrlKey) result += 2;
    if (event.altKey) result += 4;
    if (event.metaKey) result += 8;
    return result;
  }
...
var unwex_bash = function(control) {
    ...
    if (command !== undefined) {
      if (command) {
        ...
        if (commands[command[0]]) {
          ...
          control.writeAnimated(
            '-bash: ' + command[0] + ': command not found\n\n',
            self.prompt,
          );
          ga(
            'send',
  ...
  self.register = function(handler, data) {
    ...
    data &&
      data.commands &&
      data.commands.forEach(function(value, index) {
        commands[value] = handler;
      });
    data &&
      data.hiddenCommands &&
      data.hiddenCommands.forEach(function(value, index) {
  ...
  self.registeredCommands = function() {
    ...
    for (var i in commands) registeredCommands.push(i);
  ...
  self.registeredHiddenCommands = function() {
    ...
    for (var i in hiddenCommands) registeredHiddenCommands.push(i);
    registeredHiddenCommands.sort();
    return registeredHiddenCommands;
  };

  self.getCwd = function(relative) {
    if (relative) return cwd.replace(home, '~');
    return cwd;
  };

  self.setCwd = function(nwd) {
    if (nwd.indexOf('~') == 0) nwd = nwd.replace(/^~/, home);
    cwd = nwd;
    return cwd;
  };
  ...
  self.resolvePwd = function(pwd) {
    ...
    if (pwd == '..' && self.getCwd() == '/') pwd = '/';

    if (pwd.charAt(0) != '/') pwd = self.getCwd() + '/' + pwd;

    if (pwd.charAt(0) == '~') pwd = pwd.replace(/^~/, self.getHome());

    while (pwd.match(re)) pwd = pwd.replace(re, '');

    if (pwd.charAt(0) != '/') pwd = '/' + pwd;
...
var unwex_fs = function(bash, control) {
  var self = this,
    ...
    fs = {
      ...
      '/home/wes/projects/booking.com.txt': {
        t: 'f',
        d: initDate,
        u: 'wes    wes   ',
        m: '-rw-r--r--',
      ...
      '/home/wes/projects/mundo-dbz.txt': {
        t: 'f',
        d: initDate,
        u: 'wes    wes   ',
        m: '-rw-r--r--',
      ...
      '/home/wes/projects/palavrometro.txt': {
        t: 'f',
        d: initDate,
        u: 'wes    wes   ',
        m: '-rw-r--r--',
      ...
      '/home/wes/projects/prefixo9.txt': {
        t: 'f',
        d: initDate,
        u: 'wes    wes   ',
        m: '-rw-r--r--',
      ...
      '/home/wes/projects/univision.txt': {
        t: 'f',
        d: initDate,
        u: 'wes    wes   ',
        m: '-rw-r--r--',
      ...
      '/home/wes/projects/whocares.txt': {
        t: 'f',
        d: initDate,
        u: 'wes    wes   ',
        m: '-rw-r--r--',
      ...
      '/media/cdrom/Track 01 - One More Time.cda': {
        t: 'f',
        d: initDate,
        u: 'root   root  ',
        m: '-rw-r--r--',
      ...
      '/media/cdrom/Track 02 - Aerodynamic.cda': {
        t: 'f',
        d: initDate,
        u: 'root   root  ',
        m: '-rw-r--r--',
      ...
      '/media/cdrom/Track 03 - Digital Love.cda': {
        t: 'f',
        d: initDate,
        u: 'root   root  ',
        m: '-rw-r--r--',
      ...
      '/media/cdrom/Track 04 - Harder Better Faster Stronger.cda': {
        t: 'f',
        d: initDate,
        u: 'root   root  ',
        m: '-rw-r--r--',
      ...
      '/media/cdrom/Track 05 - Crescendolls.cda': {
        t: 'f',
        d: initDate,
        u: 'root   root  ',
        m: '-rw-r--r--',
      ...
      '/media/cdrom/Track 06 - Nightvision.cda': {
        t: 'f',
        d: initDate,
        u: 'root   root  ',
        m: '-rw-r--r--',
      ...
      '/media/cdrom/Track 07 - Superheroes.cda': {
        t: 'f',
        d: initDate,
        u: 'root   root  ',
        m: '-rw-r--r--',
      ...
      '/media/cdrom/Track 08 - High Life.cda': {
        t: 'f',
        d: initDate,
        u: 'root   root  ',
        m: '-rw-r--r--',
      ...
      '/media/cdrom/Track 09 - Something About Us.cda': {
        t: 'f',
        d: initDate,
        u: 'root   root  ',
        m: '-rw-r--r--',
      ...
      '/media/cdrom/Track 10 - Voyager.cda': {
        t: 'f',
        d: initDate,
        u: 'root   root  ',
        m: '-rw-r--r--',
      ...
      '/media/cdrom/Track 11 - Veridis Quo.cda': {
        t: 'f',
        d: initDate,
        u: 'root   root  ',
        m: '-rw-r--r--',
      ...
      '/media/cdrom/Track 12 - Short Circuit.cda': {
        t: 'f',
        d: initDate,
        u: 'root   root  ',
        m: '-rw-r--r--',
      ...
      '/media/cdrom/Track 13 - Face to Face.cda': {
        t: 'f',
        d: initDate,
        u: 'root   root  ',
        m: '-rw-r--r--',
      ...
      '/media/cdrom/Track 14 - Too Long.cda': {
        t: 'f',
        d: initDate,
        u: 'root   root  ',
        m: '-rw-r--r--',
  ...
  self.load = function() {
    ...
    setTimeout(function() {
      bash
        .registeredCommands()
        .concat(bash.registeredHiddenCommands())
        .sort()
    ...
    switch (command) {
      case 'cat':
        if (!options[0])
          control.writeAnimated(
            "Here's a cat:\n" +
              catCats[parseInt(Math.random() * catCats.length)],
            self.terminate,
          );
        else {
          ...
          if (!fs[file] || fs[file].t != 'f') {
            control.writeAnimated(
              '-bash: cat: ' + file + ': No such file or directory',
              self.terminate,
            );
          } else if (file == '/var/www/construcao.gif') {
            control.writeAnimated(
              '-bash: cat: ' + file + ' is a binary file',
              function() {
                $('.home-hero').css(
                  'background',
                  'url(/fsf/' +
      ...
      case 'cd':
        ...
        if (!option) {
          ...
          if (fs[option] && fs[option].t == 'd') nwd = option;
          else error = true;
        }
        ...
        if (error === 1)
          control.writeAnimated(
            '-bash: cd: ' + options[0] + ': No such file or directory',
            self.terminate,
          );
        else if (error === 2) {
          history.back();
      ...
      case 'ls':
        ...
        options.forEach(function(value, index) {
          if (value.indexOf('-') == 0 && value.indexOf('--') != 0) {
            if (value.indexOf('l') != -1) detailed = true;
            if (value.indexOf('a') != -1) all = true;
            if (value.indexOf('h') != -1) human = true;
          } else {
        ...
        if (!fs[cwd] || fs[cwd].t != 'd') {
          control.writeAnimated(
            '-bash: ls: cannot access ' + cwd + ': No such file or directory',
            self.terminate,
          );
        } else {
          if (all) {
            ...
            if (detailed) {
              list.forEach(function(item, i) {
                listOut +=
                  ...
                  item[2].u +
                  ' ' +
                  ' '.repeat(maxSizeLen - item[2].s.toString().length) +
                  item[2].s.toString() +
                  ' ' +
                  formatDate(item[2].d) +
                  ' ' +
              ...
              list.forEach(function(item, i) {
                listOut += item[0];
                if ((i + 1) % columns == 0) listOut += '\n';
                else listOut += ' '.repeat(maxNameLen - item[0].length + 2);
              });
            }
      ...
      case 'touch':
        control.writeAnimated(
          'Push me\r, and then just touch me\r\nTill I can get my\r satisfaction',
          self.terminate,
        );
        return;
      ...
      default:
        control.writeAnimated(
          '-fs: ' + command + ': command not yet implemented',
          self.terminate,
        );
        return;
    }
...
var unwex_utils = function(bash, control) {
  var self = this,
    ...
    whoamiPhrases = [
      'I have no idea.',
      "Really, I don't know.",
      "Come on, believe me, I can't even see you.",
      'Okay, enable your webcam.',
      "Didn't work. Try again.",
      'One more time, try unplugging and plugging it again.',
      'Have you tried restarting your modem?',
      'Go to Internet Options > Empty Temporary Internet Files.',
      "Nevermind, I can't help.",
      'Please, bear with me...',
  ...
  self.run = function(command, options) {
    switch (command) {
      ...
      case 'date':
        ...
        else control.writeAnimated('Today.', self.terminate);
        return;
      ...
      case 'help':
        ...
        commands.forEach(function(command, index) {
          help += command;
          if (index % 2 == 1) help += '\n';
          else help += ' '.repeat(20 - command.length);
        });
      ...
      case 'man':
        control.writeAnimated(
          manPhrases[parseInt(Math.random() * manPhrases.length)],
          self.terminate,
        );
        return;
                ...
                function(newPasswd1) {
                  if (newPasswd1.match(/(123|abc|qwe|asd|password|adobe)/i))
                    ...
                    control.scanf(
                      ...
                      function(newPasswd2) {
                        control.writeAnimated(
                          '-' + command + ': Authentication token failure',
                          self.terminate,
                        );
                      },
                      { invisible: true },
      ...
      case 'sudo':
        control.writeAnimated(
          'Unauthorized usage, please wait until the FBI arrives at your door.',
          self.terminate,
        );
        return;
      ...
      case 'whoami':
        if (whoamiPhraseCurrent >= whoamiPhrases.length)
          control.writeAnimated(
            '-bash: ' + command + ': gave up and killed itself',
            self.terminate,
          );
        else {
          control.writeAnimated(
            whoamiPhrases[whoamiPhraseCurrent],
            self.terminate,
          );
          whoamiPhraseCurrent++;
        }
      ...
      case 'whereis':
        control.writeAnimated(
          whereisPhrases[parseInt(Math.random() * whereisPhrases.length)],
          self.terminate,
        );
        return;
      ...
      default:
        control.writeAnimated(
          '-utils: ' + command + ': command not yet implemented',
          self.terminate,
        );
        return;
    }
  ...
  self.run = function(command, options) {
    switch (command) {
      ...
      case 'hello':
        ...
        else control.writeAnimated('Hi there.', self.terminate);
        return;
      ...
      case 'iexplorer.exe':
        ...
        } else control.bsod(iexplorerBsod);
        return;

      case 'make':
        if (options.length == 0)
          control.writeAnimated(
            '-' + command + ': nothing to make',
            self.terminate,
          );
        else if (
          options.length == 3 &&
          options.join('').toLowerCase() == 'measandwich'
        )
          control.writeAnimated(
            "Okay.\r\r\r See? I'm a nice guy.\r\r\n\nYour sandwich will arrive in 0 to undefined business days.",
            self.terminate,
          );
        else
          control.writeAnimated(
            '-' + command + ": I can't make this",
            self.terminate,
          );
        return;
      ...
      case 'why':
        control.writeAnimated(
          whyPhrases[parseInt(Math.random() * whyPhrases.length)],
          self.terminate,
        );
        return;
      ...
      case 'win':
        control.writeAnimated(
          "Really?\r REALLY YOU THINK I'M DOS??\r\r\n\nFUCK YOU!\r\r\n\nENJOY THIS TERRIFIC WINDOWS EXPERIENCE IN 3...\r\r 2...\r\r 1\r",
          function() {
      ...
      default:
        control.writeAnimated(
          '-bug: ' +
            command +
            ': command not yet implemented because I forgot, please e-mail me asking for it',
          self.terminate,
        );
        return;
    }
...
function debug() {
  if (location.hostname == 'localhost') console.log.apply(console, arguments);
}
