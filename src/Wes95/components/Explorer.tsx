...
export const Explorer = () => {
  ...
  const [poweredOff, setPoweredOff] = createSignal(false);
  ...
  return (
    <ErrorBoundary
      fallback={(error, reset) => <BSOD error={error} reset={reset} />}
    >
      <FocusProvider>
        <Show when={!poweredOff()} fallback={<SafeToPowerOff />}>
          <div
            classList={{
              Screen: true,
...
function BSOD(p: { error: any; reset: () => void }) {
  return (
    <button class="BSOD GhostButton" onClick={p.reset} type="button">
      ...
      <div class="BSODMessage">
        <p>
          A fatal exception occurred and the website has halted.{' '}
          {String(p.error).replace(/^[a-z]*Error: /i, '')}. The current
          application will be terminated.
        </p>
        <p>
          Click anywhere to continue <span class="Blinking">_</span>
        </p>
      </div>
    </button>
  );
...
function SafeToPowerOff() {
  return (
    <div class="SafeToPowerOff">
      <span>It's now safe to navigate to another website.</span>
    </div>
  );
}
